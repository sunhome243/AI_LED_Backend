...
resource "aws_lambda_permission" "isConnect_api_permission" {
  ...
  function_name = element(split(":", var.isConnect_lambda_arn), length(split(":", var.isConnect_lambda_arn)) - 1)
...
resource "aws_lambda_permission" "pattern_to_ai_api_permission" {
  ...
  action        = "lambda:InvokeFunction"
  function_name = element(split(":", var.pattern_to_ai_lambda_arn), length(split(":", var.pattern_to_ai_lambda_arn)) - 1)
  principal     = "apigateway.amazonaws.com"
  # More specific source ARN for this endpoint
...
resource "aws_lambda_permission" "audio_to_ai_api_permission" {
  ...
  action        = "lambda:InvokeFunction"
  function_name = element(split(":", var.audio_to_ai_lambda_arn), length(split(":", var.audio_to_ai_lambda_arn)) - 1)
  principal     = "apigateway.amazonaws.com"
  # More specific source ARN for this endpoint
