...
resource "aws_api_gateway_integration" "pattern_to_ai_api_int" {
  ...
  type        = "AWS_PROXY"
  uri         = var.pattern_to_ai_lambda_arn  # Use the invoke ARN directly
}
...
resource "aws_api_gateway_integration" "audio_to_ai_api_int" {
  ...
  integration_http_method = "POST"
  type        = "AWS_PROXY"
  uri         = var.audio_to_ai_lambda_arn  # Use the invoke ARN directly
}
...
resource "aws_api_gateway_integration" "is_connect_api_int" {
  ...
  integration_http_method = "POST"
  type        = "AWS_PROXY"
  uri         = var.isConnect_lambda_arn  # Use the invoke ARN directly
}
...
resource "aws_lambda_permission" "pattern_to_ai_api_permission" {
  ...
  principal     = "apigateway.amazonaws.com"
  # Fix source ARN format to allow API Gateway to invoke Lambda
  source_arn    = "${aws_api_gateway_rest_api.rest_api.execution_arn}/*/*/pattern_to_ai/create"
...
resource "aws_lambda_permission" "audio_to_ai_api_permission" {
  ...
  principal     = "apigateway.amazonaws.com"
  # Fix source ARN format to allow API Gateway to invoke Lambda
  source_arn    = "${aws_api_gateway_rest_api.rest_api.execution_arn}/*/*/audio_to_ai/create"
